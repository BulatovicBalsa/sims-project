...
public class PrescriptionTypeConverter : DefaultTypeConverter
{
    public override object ConvertFromString(string text, IReaderRow row, MemberMapData memberMapData)
    {
        ...
        if (prescriptionArgs.Length != 6) throw new FormatException("Invalid prescription format.");
    ...
    public override string ConvertToString(object value, IWriterRow row, MemberMapData memberMapData)
    {
        if (value is Prescription prescription)
            return
                $"{prescription.Medication.Id};{prescription.Amount};{prescription.DailyUsage};{prescription.MedicationTiming};{prescription.IssuedDate.ToString("yyyy-MM-dd HH:mm:ss")};{prescription.DoctorId}";
