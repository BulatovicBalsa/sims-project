...
public sealed class ExaminationWriteMapper : ClassMap<Examination>
{
    public ExaminationWriteMapper()
    {
        ...
        Map(examination => examination.Admissioned).Index(7);
        Map(examination => examination.Urgent).Index(8);
        Map(examination => examination.ProcedureDoctors).Index(9).Convert(row =>
            string.Join("|", row.Value.ProcedureDoctors?.Select(doctor => doctor.Id) ?? new List<string>())).Index(9);
        Map(examination => examination.ProcedureNurses).Index(10).Convert(row =>
            string.Join("|", row.Value.ProcedureNurses?.Select(nurse => nurse.Id) ?? new List<string>())).Index(10);
    }
}
...
public class ExaminationRepository
{
    ...
    {
        foreach (var examination in GetExaminationsInTimeRange(doctor, timeRange)) Delete(examination, false);
    }
    ...
    public List<Examination> GetFinishedExaminations(Doctor doctor)
    {
        ...
        var finishedExaminations = GetAll()
            .Where(examination => (examination.Doctor?.Equals(doctor) ?? false) && examination.Start < currentTime)
            .ToList();
        return finishedExaminations;
    }

    public List<Examination> GetExaminationsForDate(Doctor doctor,
    ...
    public List<Examination> GetExaminationsInTimeRange(Doctor doctor,
        TimeRange range)
    {
        return GetAll(doctor)
            .Where(examination => range.DoesOverlapWith(new TimeRange(examination.Start, examination.End))).ToList();
    }

    public List<Examination> GetExaminationsForDate(Patient patient,
        DateTime requestedDate)
    {
