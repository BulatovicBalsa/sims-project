...
public class PatientCli
{
    ...
    private void GetRecommendedExaminations()
    {
        ...
        var options = new ExaminationSearchOptions(selectedDoctor, latestDate.Value, startTime.Value, endTime.Value,
            priority.Value);
    ...
    private Doctor GetSelectedDoctor()
    {
        ...
        allDoctors.Select((doctor, index) => $"{index + 1}. {doctor.FirstName} {doctor.LastName}").ToList()
    ...
    private Examination GetExaminationSelection(List<Examination> availableExaminations)
    {
        while (true)
        {
            ...
            if (!int.TryParse(Console.ReadLine(), out var examinationSelection) || examinationSelection < 1 ||
                examinationSelection > availableExaminations.Count)
            {
